$(document).ready(function () {
	...
	var table = $('#example').DataTable({
		...
		"processing": true,
		ajax: {
			url: _url + 'customerlog/getlistlogrecente',
			"type": "GET",
			"data": function (d) {
				return $.extend({}, d, {
					"kit": $("select[id=kit]").val(),
					"com": $("select[id=com]").val(),
					"statut": $("select[id=statut]").val(),
		...
		"columns": [
			...
			{title: "Total", data: "sommeTotalckph"},
		...
		"columnDefs": [{
			...
			render: function (data, type, row, meta) {
				return '<div class="dropdown"> ' +
					...
					'<li>' +
					//'<a style="color:black;" target="_blank" href="' + _url + 'customer/viewcustomerkitpaiementlog?id=' + row.idCustomer + '&idcustkit='+row.idCustomerKit+'&idcustkitpay='+ row.idcustomerkitpaiement+'"><i class="fa fa-eye"> </i> Détails</a> ' +
					'<a style="color:black;" target="_blank" href="' + _url + 'customer/viewcustkitpaiement?id=' + row.idCustomer + '&idcustkit='+row.idCustomerKit+'"><i class="fa fa-eye"> </i> Détails</a> ' +
					'</li> ' +
					'</ul> ' +
					'</div>';
			}
